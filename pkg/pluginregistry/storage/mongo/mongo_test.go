...
func TestPlugin_Json2MockData(t *testing.T) {
	jsonStr := `{"uniqueKey":"advanced_example","path":"/adv/Hello","method":["POST","GET"],"cases":[{"condition":{"simple":{"items":[{"operandX":"$request.body.uid","operator":"<=","operandY":"1000"}]}},"response":{"simple":{"body":{"x-unit-id":"3","x-unit-region":"sh"},"trailer":{"x-api-version":"1.3.2"},"body":"{\"timestamp\": \"1111\", \"message\": \"This message will only be returned when uid <= 1000\", \"amount\": \"{{ $mock.price }}\"}\n"}}},{"condition":{"simple":{"items":[{"operandX":"$request.body.uid","operator":">","operandY":"1000"}]}},"response":{"script":{"lang":"javascript","content":"(function(){\n    function random(min, max){\n        return parseInt(Math.random()*(max-min+1)+min,10);\n    }\n    return {\n        code: 0,\n        body: {\n            \"x-unit-id\": (request.body[\"uid\"] % 5).toString(),\n            \"x-unit-region\": \"bj\",\n        },\n        trailer: {\n            \"x-api-version\": \"1.3.2\",\n        },\n        body: {\n            timestamp: Math.ceil(new Date().getTime() / 1000),\n            message: \"this message is generated by javascript, your uid is: \" + request.body[\"uid\"],\n            amount: random(0, 5000),\n        },\n    }\n})()"}}}]}`
	var mockDataInput MockData
	err := json.Unmarshal([]byte(jsonStr), &mockDataInput)
	if err != nil {
		fmt.Println("Error:", err)
		return
	}

	fmt.Printf("%+v", mockDataInput)
}
